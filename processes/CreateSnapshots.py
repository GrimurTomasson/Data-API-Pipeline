...
def create_schema_if_missing (conn, target_schema):
    erTil = conn.cursor ().execute ('SELECT COUNT(1) AS er_til FROM sys.schemas s WHERE s.name = ?', target_schema).fetchval ()
    if (erTil == 1):
        print (f'Target schema ({target_schema}) already exists')
    else:
        print (f'Target schema ({target_schema}) missing, creating it')
        conn.cursor ().execute (f'CREATE SCHEMA {target_schema}')
...
def create_missing_views (conn, snapshot_schema, history_schema):
    ...
    create_schema_if_missing (conn, history_schema)
...
def create_snapshots (database_server, database_name, source_schema, snapshot_schema, history_schema, snapshotDateColumnName ):
    ...
    create_schema_if_missing (conn, snapshot_schema)
    print (APISupport.separator)
