...
class ConceptGlossaryRvk (ConceptGlossary):
    glossaryQuery = """
        ...
        FROM
            ...
            LEFT JOIN [MASTER-API].Nustada@Hugtak.hugtak_utfaersla dd ON dd.gagnaform_id = 'SQL_SERVER' AND dd.heiti = c.COLUMN_NAME
            LEFT JOIN [MASTER-API].Nustada@Hugtak.hugtak bg ON bg.id = dd.hugtak_id
        WHERE
            c.TABLE_CATALOG = ?
        ORDER BY 
