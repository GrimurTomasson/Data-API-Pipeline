...
{% macro sqlserver__get_catalog(information_schemas, schemas) -%}
  ...
  {%- call statement('catalog', fetch_result=True) -%}
    ...
    principals as (
        ...
        from
            [{{ information_schemas.database }}].sys.database_principals {{ information_schema_hints() }}
    ),
    ...
    schemas as (
        select
            ...
            principal_id as principal_id
        from
            [{{ information_schemas.database }}].sys.schemas {{ information_schema_hints() }}
    ),
    ...
    tables as (
        ...
        from
            [{{ information_schemas.database }}].sys.tables {{ information_schema_hints() }}
    ),
    ...
    views as (
        select
            ...
            'VIEW' as table_type
        from
            [{{ information_schemas.database }}].sys.views {{ information_schema_hints() }}
    ),
    ...
    cols as (
        ...
        from 
          [{{ information_schemas.database }}].INFORMATION_SCHEMA.COLUMNS {{ information_schema_hints() }}
