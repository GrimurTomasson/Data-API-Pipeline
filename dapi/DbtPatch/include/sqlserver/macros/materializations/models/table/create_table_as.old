...
{% macro sqlserver__create_table_as(temporary, relation, sql) -%}
   ...
   {% set contract_config = config.get('contract') %}
    ...
    {%- else %}
      ...
      {% set index_name = (relation.schema ~ '_' ~ relation.identifier ~ '_cci') | replace(' ', '_')  %}
      EXEC('CREATE CLUSTERED COLUMNSTORE INDEX {{index_name}} ON [{{relation.database}}].[{{relation.schema}}].[{{relation.identifier}}]')
    {% endif %}
