...
class BaseAdapter(metaclass=AdapterMeta):
    ...
    def _get_one_catalog(
        ...
        radir = 0 if table.rows is None else len(table.rows)
        print (f"_get_one_catalog - info: {information_schema} - rows: {radir}")
        
        results = self._catalog_filter_table(table, manifest)
        radir = 0 if results.rows is None else len(results.rows)
        print (f"_get_one_catalog - info: {information_schema} - filtered rows: {radir}")
    ...
    def get_catalog(self, manifest: Manifest) -> Tuple[agate.Table, List[Exception]]:
        ...
        with executor(self.config) as tpe:
            ...
            catalogRadir = 0 if catalogs.rows is None else len(catalogs.rows)
            exceptionRadir = 0 if exceptions is None else len(exceptions)
            print (f"Samtals catalog radir: {catalogRadir} - Villur: {exceptionRadir}")
